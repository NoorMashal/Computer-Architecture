...
void accessData (
    ...
    /* ... */
    for (int i = 0; i < E; i++) {
        if (cache[setInd][i].valid && cache[setInd][i].tag == newT) 
        {
            (*hit_count)+=1;                                    // increment hit_count by 1.
            cache[setInd][i].recent_use = program_counter;
            return;
        }
    }
    ...
    /* ... */
    size_t min = cache[setInd][0].recent_use;
    int LRU = 0;
    for (int i = 1; i < E; i++) {
        if (cache[setInd][i].recent_use < min) {
            min = cache[setInd][i].recent_use;
            LRU = i;
        }
    }
    // If cache set line already in use, then record an eviction
    /* ... */
    if (cache[setInd][LRU].valid) {
        cache[setInd][LRU].tag = newT;
         cache[setInd][LRU].recent_use = program_counter;
        (*eviction_count)+=1;
    } else {
        cache_line_t newI;
        newI.valid = true;
        newI.tag = newT;
        newI.recent_use = program_counter;
        cache[setInd][LRU] = newI;
    }
    /* ... */
}
