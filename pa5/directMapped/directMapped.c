...
void accessData (
    ...
    int setindex = addr >> b & 15;

    // Cache hit
    /* ... */
    if (cache[setindex].valid && cache[setindex].tag == newT) {
        (*hit_count)++;
        return;
    }
    ...
    {
        cache_set_line_t newI;               // Creates a new item that will be added.
        newI.valid = true;                   // Validates new iten
        newI.tag = newT;                     
        cache[setindex] = newI;              // Adds the new item to the cache
    } else {
        cache[setindex].tag = newT;
        (*eviction_count)++;
    }
